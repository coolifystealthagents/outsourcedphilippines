...
EXPECTED_TITLE = 'Philippines remote staff data access checklist'
EXPECTED_CANONICAL = 'https://outsourcedphilippines.com/blog/philippines-remote-staff-data-access-checklist'
EXPECTED_MARKER = 'ph-remote-access-checklist-v1'
EXPECTED_QUOTE = 'Zero trust assumes there is no implicit trust granted to assets or user accounts based solely on their physical or network location (i.e., local area networks versus the internet) or based on asset ownership (enterprise or personally owned).'
...
class AuditParser(HTMLParser):
    def __init__(self):
        ...
        self.counts = {'banners': 0, 'tables': 0, 'evidence_charts': 0, 'access_graphics': 0, 'methods_notes': 0, 'source_items': 0, 'source_numbers': 0}
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == 'svg' and 'access-flow' in classes:
            self.counts['access_graphics'] += 1
...
def collect_types(node, found):
    if isinstance(node, dict):
        ...
        elif isinstance(value, list): found.update(v for v in value if isinstance(v, str))
        for child in node.values(): collect_types(child, found)
    elif isinstance(node, list):
        for child in node: collect_types(child, found)
...
def main():
    ...
    for item in parser.jsonld:
        try: collect_types(json.loads(item), schema_types)
        except Exception as exc: schema_errors.append(str(exc))
    forbidden_terms = {term: len(re.findall(rf'\b{term}\b', document, re.I)) for term in ('pricing', 'rates', 'tiers')}
    ...
    checks = {
        ...
        'table': parser.counts['tables'] >= 1,
        'labeled_chart': parser.counts['evidence_charts'] == 1 and 'risk-chart-title' in raw and 'risk-chart-desc' in raw,
        'separate_graphic': parser.counts['access_graphics'] == 1 and 'access-flow-title' in raw and 'access-flow-desc' in raw,
        'methods_note': parser.counts['methods_notes'] == 1,
        'dated_stats': all(marker in article for marker in ('68%', '28%', '15%', '193,407')),
        'exact_quote': EXPECTED_QUOTE in article,
