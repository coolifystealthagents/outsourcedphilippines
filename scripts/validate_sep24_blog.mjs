...
const source=fs.readFileSync('app/blog/sep24-blog-batch.ts','utf8');
...
if(!source.includes("publishedAt='2026-09-24'"))throw new Error('publication date mismatch');
const allFiles=fs.readdirSync('app/blog').filter(x=>x.endsWith('.ts')).filter(x=>x!=='sep24-blog-batch.ts').map(x=>fs.readFileSync(path.join('app/blog',x),'utf8')).join('\n');
for(const slug of slugs){
 ...
 if(!html.includes('2026-09-24')||!html.includes('datePublished'))throw new Error(`${slug}: publication metadata missing`);
