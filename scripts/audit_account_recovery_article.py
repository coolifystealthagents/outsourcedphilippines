...
EXPECTED_TITLE = 'Philippines outsourced customer support account recovery checklist'
EXPECTED_CANONICAL = 'https://outsourcedphilippines.com/blog/philippines-outsourced-customer-support-account-recovery-checklist'
EXPECTED_MARKER = 'ph-account-recovery-support-v1'
EXPECTED_QUOTE = 'An account recovery event always causes one or more notifications to be sent to the subscriber to help detect the fraudulent use of account recovery.'
...
class AuditParser(HTMLParser):
    def __init__(self):
        ...
        self.counts = {'banners': 0, 'tables': 0, 'evidence_charts': 0, 'recovery_graphics': 0, 'methods_notes': 0, 'source_items': 0, 'source_numbers': 0, 'blockquotes': 0}
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == 'svg' and 'account-recovery-flow' in classes:
            self.counts['recovery_graphics'] += 1
...
def main():
    ...
    checks = {
        ...
        'one_labeled_chart': parser.counts['evidence_charts'] == 1 and 'recovery-chart-title' in raw and 'recovery-chart-desc' in raw,
        'one_separate_graphic': parser.counts['recovery_graphics'] == 1 and 'recovery-flow-title' in raw and 'recovery-flow-desc' in raw,
        'methods_note': parser.counts['methods_notes'] == 1,
        'dated_stats': all(marker in article for marker in ('2024', '193,407', '64,882', '21,403', '982', '2.6 million', '$12.5 billion', '$789 million')),
        'scope_caveats': all(marker in article for marker in ('U.S. federal system', 'do not measure account attacks handled by Filipino workers', 'context, not a forecast')),
        'exact_quote': EXPECTED_QUOTE in article and parser.counts['blockquotes'] == 1,
        'quote_attribution': all(marker in article for marker in ('NIST Special Publication 800-63B', 'August 26, 2025')),
