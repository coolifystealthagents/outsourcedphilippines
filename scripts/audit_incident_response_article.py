...
EXPECTED_TITLE = 'Philippines outsourced support incident response checklist'
EXPECTED_CANONICAL = 'https://outsourcedphilippines.com/blog/philippines-outsourced-support-incident-response-checklist'
EXPECTED_MARKER = 'ph-outsourced-support-incident-response-v1'
EXPECTED_QUOTE = 'Regardless of the incident response life cycle framework or model used, every organization should take incident response into consideration throughout their cybersecurity risk management activities.'
...
class AuditParser(HTMLParser):
    def __init__(self):
        ...
        self.counts = {'banners': 0, 'tables': 0, 'evidence_charts': 0, 'incident_graphics': 0, 'methods_notes': 0, 'source_items': 0, 'source_numbers': 0, 'blockquotes': 0}
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == 'svg' and 'incident-flow' in classes:
            self.counts['incident_graphics'] += 1
...
def main():
    ...
    source_urls = [
        ...
        'https://privacy.gov.ph/wp-content/uploads/2016/12/NPC-Circular-16-03-Personal-Data-Breach-Management.pdf',
    ]
    checks = {
        ...
        'one_labeled_chart': parser.counts['evidence_charts'] == 1 and 'incident-chart-title' in raw and 'incident-chart-desc' in raw,
        'one_separate_graphic': parser.counts['incident_graphics'] == 1 and 'incident-flow-title' in raw and 'incident-flow-desc' in raw,
        'methods_note': parser.counts['methods_notes'] == 1,
        'dated_stats': all(marker in article for marker in ('22,052', '12,195', '60%', '30%', '20%', '193,407')),
