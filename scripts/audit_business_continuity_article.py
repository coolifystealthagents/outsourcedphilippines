...
EXPECTED_TITLE = 'Philippines remote team business continuity checklist'
EXPECTED_CANONICAL = 'https://outsourcedphilippines.com/blog/philippines-remote-team-business-continuity-checklist'
EXPECTED_MARKER = 'ph-remote-team-continuity-v1'
EXPECTED_QUOTE = 'Contingency planning refers to interim measures to recover information system services after a disruption.'
...
class AuditParser(HTMLParser):
    def __init__(self):
        ...
        self.counts = {'banners': 0, 'tables': 0, 'evidence_charts': 0, 'continuity_graphics': 0, 'methods_notes': 0, 'source_items': 0, 'source_numbers': 0, 'blockquotes': 0}
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == 'svg' and 'continuity-flow' in classes:
            self.counts['continuity_graphics'] += 1
...
def main():
    ...
    source_urls = [
        ...
        'https://data.worldbank.org/indicator/IT.NET.BBND.P2?locations=PH',
    ...
    checks = {
        ...
        'one_labeled_chart': parser.counts['evidence_charts'] == 1 and 'continuity-chart-title' in raw and 'continuity-chart-desc' in raw,
        'one_separate_graphic': parser.counts['continuity_graphics'] == 1 and 'continuity-flow-title' in raw and 'continuity-flow-desc' in raw,
        'methods_note': parser.counts['methods_notes'] == 1,
        'dated_stats': all(marker in article for marker in ('2024', '67.3%', '7.1', '115.3')),
        'philippines_weather_context': all(marker in article for marker in ('20 tropical cyclones', '8 or 9', '70%')),
        'exact_quote': EXPECTED_QUOTE in article and parser.counts['blockquotes'] == 1,
        'quote_attribution': all(marker in article for marker in ('Marianne Swanson', 'Pauline Bowen', 'Amy Wohl Phillips', 'Dean Gallup', 'David Lynes')),
