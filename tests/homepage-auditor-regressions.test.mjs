...
test('labeled homepage images are exposed to assistive technology', () => {
  const source = homepageSource();
  assert.ok(!/<(?:Image|img)\b(?=[^>]*\balt=(?:"[^"]+"|'[^']+'|\{`[^`]+`\}))(?=[^>]*\baria-hidden="true")[^>]*>/s.test(source));
  assert.ok(!source.includes('className="role-card-art" aria-hidden="true"'));
});
